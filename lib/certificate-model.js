...
class CertificateModel {
	...
	constructor(model) {
		...
		if (model['subjectName']) {
			this._subjectName = new NameModel(model['subjectName']);
		}
		if (model['issuerName']) {
			this._issuerName = new NameModel(model['issuerName']);
		}
		if (model['pkiBrazil']) {
			this._pkiBrazil = new PkiBrazilCertificateModel(model['pkiBrazil']);
		}
		if (model['pkiItaly']) {
			this._pkiItaly = new PkiItalyCertificateModel(model['pkiItaly']);
		}
		if (model['issuer']) {
			this._issuer = new CertificateModel(model['issuer']);
		}
...
exports.PkiBrazilCertificateModel = PkiBrazilCertificateModel;
exports.PkiItalyCertificateModel = PkiItalyCertificateModel;
