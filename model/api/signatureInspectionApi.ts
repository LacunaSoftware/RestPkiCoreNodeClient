...
export enum SignatureInspectionApiApiKeys {
}
...
export class SignatureInspectionApi {
    ...
    public setApiKey(key: SignatureInspectionApiApiKeys, value: string) {
        (this.authentications as any)[SignatureInspectionApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param inspectSignatureRequest 
     */
    public async apiSignatureInspectionPut (inspectSignatureRequest?: InspectSignatureRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: InspectSignatureResponse;  }> {
        const localVarPath = this.basePath + '/api/signature-inspection';
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(inspectSignatureRequest, "InspectSignatureRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: InspectSignatureResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "InspectSignatureResponse");
